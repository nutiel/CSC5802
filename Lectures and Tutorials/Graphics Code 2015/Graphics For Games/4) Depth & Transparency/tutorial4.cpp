...
int main()	{
	...
	while(w.UpdateWindow() && !Window::GetKeyboard()->KeyDown(KEYBOARD_ESCAPE)){				//And enter a while loop that renders the scene
		if(Window::GetKeyboard()->KeyTriggered(KEYBOARD_1)) {
			cout << "ToggleObject: " << renderer.ToggleObject() << endl;
		}
		if(Window::GetKeyboard()->KeyTriggered(KEYBOARD_2)) {
			cout << "ToggleDepth: " << renderer.ToggleDepth() << endl;
		}
		if(Window::GetKeyboard()->KeyTriggered(KEYBOARD_3)) {
			cout << "ToggleAlphaBlend: " << renderer.ToggleAlphaBlend() << endl;
		}
		if(Window::GetKeyboard()->KeyTriggered(KEYBOARD_4)) {
			cout << "ToggleBlendMode: " << renderer.ToggleBlendMode() << endl;
		}
