...
Mesh * Mesh::GenerateTriangle(float x, float y, float z) {
	//0.0f, 0.5f, 0.0f
	...
	m->vertices[1] = Vector3(x + 0.5f, y - 1.0f, z);
	m->vertices[2] = Vector3(x - 0.5f, y - 1.0f, z);
...
Mesh * Mesh::GenerateSquare(float x, float y, float z) {
	Mesh * m = new Mesh();
	
	m->numVertices = 6;

	m->vertices = new Vector3[m->numVertices];
	m->vertices[0] = Vector3(x - 0.5f, y - 0.5f, z);
	m->vertices[1] = Vector3(x - 0.5f, y + 0.5f, z);
	m->vertices[2] = Vector3(x + 0.5f, y - 0.5f, z);
	m->vertices[3] = Vector3(x - 0.5f, y + 0.5f, z);
	m->vertices[4] = Vector3(x + 0.5f, y - 0.5f, z);
	m->vertices[5] = Vector3(x + 0.5f, y + 0.5f, z);

	m->textureCoords = new Vector2[m->numVertices];
	m->textureCoords[0] = Vector2(x - 0.0f, y + 1.0f);
	m->textureCoords[1] = Vector2(x - 0.0f, y - 0.0f);
	m->textureCoords[2] = Vector2(x + 1.0f, y + 1.0f);
	m->textureCoords[3] = Vector2(x - 0.0f, y - 0.0f);
	m->textureCoords[4] = Vector2(x + 1.0f, y + 1.0f);
	m->textureCoords[5] = Vector2(x + 1.0f, y - 0.0f);
...
void Mesh::Draw() {
	...
	if (bufferObject[INDEX_BUFFER]) {// Added by the index buffers tut ...
		...
		glDrawArrays(type, 0, numVertices);//type to draw, where to start(index: 0), how many(all of them)
	}
	glBindVertexArray(0);
	...
	glBindVertexArray(0);*/
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, 0);//releases texture
...
Mesh * Mesh::GeneratePyramid() {
	...
	m->vertices[1] = Vector3(0.5f, 1.0f, 0.5f);
	m->vertices[2] = Vector3(0.5f, 1.0f, 0.5f);
	m->vertices[3] = Vector3(1.0f, 0.0f, -1.0f);
	m->vertices[4] = Vector3(-1.0f, 0.0f, 1.0f);
	m->vertices[5] = Vector3(0.5f, 1.0f, 0.5f);
	m->vertices[6] = Vector3(0.5f, 1.0f, 0.5f);
